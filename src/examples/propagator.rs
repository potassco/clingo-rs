...
unsafe extern "C" fn init(init: *mut clingo_propagate_init_t,
                          ...
                          -> bool {
    let mut propagator = data as *mut propagator_t;
    ...
    // let mut atoms: *mut clingo_symbolic_atoms_t = std::ptr::null_mut(); //TODO create an atom?
    // if !clingo_propagate_init_symbolic_atoms(init, &mut atoms) {
    //     return false;
    // }
    let atoms = safe_clingo_propagate_init_symbolic_atoms(init).unwrap();
    // create place/2 signature to filter symbolic atoms with

    // let mut sig: clingo_signature_t = 0;
    // if !clingo_signature_create("place", 2, true, &mut sig) {
    //     return false;
    // }
    ...
    let atoms_it = atoms.begin(Some(sig)).unwrap();
    //     if (pass == 1) {
